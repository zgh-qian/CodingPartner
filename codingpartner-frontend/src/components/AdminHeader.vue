  ...
  <a-row id="globalHeader" align="center" :wrap="false">
    ...
    <a-space @click="doUserClick" class="user">
      ...
      <a-col class="nickname"
        ><a-link>{{ user.nickname }}</a-link></a-col
      >
      <a-col class="level">{{ user.level }}</a-col>
    </a-space>
    <a-link v-if="user.role === USER_ROLE.ADMIN" href="/">前台系统</a-link>
  </a-row>
</template>
...
import USER_ROLE from '@/constant/userRole.ts'
...
const visibleRoutes = computed(() => {
  ...
  return router.getRoutes().filter((item) => {
    // 管理员不显示用户页面
    if (
      (item.meta?.access === USER_ROLE.USER ||
        item.meta?.access === USER_ROLE.NOT_LOGIN) &&
